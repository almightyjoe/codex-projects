...
def run_collection(
    ...
    for index, target in enumerate(targets, 1):
        ...
        output_dir = make_output_dir(options.output_base, parsed["value"], batch=batch)
        ...
        export_csvs(result)
        EvidenceDatabase(output_dir / "evidence.sqlite").write_result(result)
        evidence.write_manifest()
        write_reports(result)
        result.evidence = list(evidence.records)
        results.append(result)
        if progress:
            progress(f"[{index}/{len(targets)}] Finished {target}: {output_dir}")
    return results
